...
IP = str(os.environ.get('ROBOT_IP'))
PORT = int(os.environ.get('ROBOT_PORT'))
rt = RobotTools(ip=IP, port=PORT)
...
async def receive_audio(websocket):
    ...
    while True:
        ...
        if "transcript" in response_data:
            if response_data["type"] == "response.audio_transcript.done":
                base64_audio_response = response_data["transcript"]
                if data:
                    pcm16_audio = base64_to_pcm16(data)
                    #音声データがある場合は、出力ストリームから再生
                    # WAVファイルのパラメータ
                    sample_rate = 24000  # サンプリングレート（例：44.1kHz）
                    num_channels = 1     # モノラルの場合は1、ステレオの場合は2
                    sample_width = 2     # PCM16なので2バイト（16ビット）
                    ...
                    d = rt.play_wav("output.wav")
                    motion = rt.make_beat_motion(d)
                    rt.play_motion(motion=motion)
                    time.sleep(d)
                    data = ""
        ...
        if "delta" in response_data:
            if response_data["type"] == "response.audio.delta":
                ...
                data = data + base64_audio_response
    ...
    async with websockets.connect(WS_URL, extra_headers=HEADERS) as websocket:
        ...
        init_request = {
            ...
            "response": {
                ...
                "instructions": "友達であるユーザーとタメ口で話してください。",
